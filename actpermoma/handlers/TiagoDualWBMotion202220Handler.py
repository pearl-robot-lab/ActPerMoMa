...
class TiagoDualWBHandler(TiagoBaseHandler):
    def __init__(self, move_group, use_torso, sim_config, num_envs, device, usd_path=None, intrinsics=None, voxel_size=0.10, motion_planner='pinocchio'):
        ...
        if motion_planner == 'pinocchio':
            ...
            self._ik_solver = PinTiagoIKSolver(move_group=move_group, include_torso=use_torso, include_base=True, include_head=True, max_rot_vel=100.0)
            self._ik_arm_solver = PinTiagoIKSolver(move_group=move_group, include_torso=use_torso, include_base=False, include_head=False, max_rot_vel=100.0)
    ...
    def get_ee_pose(self, joint_state=None):
        ...
        if joint_state is None:
            ...
            if self._move_group is 'arm_left':
                arm_pos, _= self.get_arms_dof_pos() # (num_dof,) each

            elif self._move_group is 'arm_right':
                _, arm_pos = self.get_arms_dof_pos() # (num_dof,) each
            else:
                raise ValueError('MotionPlanning with both arms is currently not enabled')
        ...
        if self.motion_planner is 'pinocchio': # don't forget to do .cpu().numpy() from torch tensors)
            ...
            ee_pos, ee_quat = self._ik_solver.solve_fk_tiago(joint_state[0])
    ...
    def get_frame_pose(self, frame, joint_state=None):
        ...
        if self.motion_planner == 'pinocchio': # don't forget to do .cpu().numpy() from torch tensors)
            joint_state = np.hstack([joint_state[:,:2], np.cos(joint_state[:, 2:3]), np.sin(joint_state[:, 2:3]), joint_state[:, 3:]]) # only for num_envs=1
            ee_pos, ee_quat = self._ik_solver.solve_fk_tiago(joint_state[0], frame)
    ...
    def solve_arm_ik(self, des_pos, des_quat, joint_state=None, frame=None, n_trials=7, dt=0.1, pos_threshold=0.02, angle_threshold=5.*np.pi/180, verbose=False):
        ...
        if joint_state is None:
            ...
            arm_pos = arm_pos.cpu().numpy()
            joint_state = np.hstack((torso_pos, arm_pos))
        ...
        if self.motion_planner == 'pinocchio': # don't forget to do .cpu().numpy() from torch tensors)
            ...
            success, best_q, q_list = self._ik_arm_solver.solve_ik_pos_tiago(des_pos, des_quat, joint_state[0], frame, n_trials, dt, pos_threshold, angle_threshold, verbose) # only for num_envs=1
    ...
    def solve_ik(self, des_pos, des_quat, joint_state=None, frame=None, n_trials=7, dt=0.1, pos_threshold=0.02, angle_threshold=5.*np.pi/180, verbose=False):
        # joint_state: (num_env, num_dof) tensor or ndarray; if given, will calculate ee pose for this joint state, otherwise for current joint state
        if frame == 'zed':
            frame = 'head_2_link'
        ...
        if self.motion_planner == 'pinocchio': # don't forget to do .cpu().numpy() from torch tensors)
            ...
            joint_state = np.hstack([joint_state[:,:2], np.cos(joint_state[:, 2:3]), np.sin(joint_state[:, 2:3]), joint_state[:, 3:]]) # only for num_envs=1
            success, best_q, q_list = self._ik_solver.solve_ik_pos_tiago(des_pos, des_quat, joint_state[0], frame, n_trials, dt, pos_threshold, angle_threshold, verbose)
