...
class wandbLogger:
    def __init__(self, exp_config, run_name=None, group_name=None, disabled=False):
        if disabled:
            mode="disabled"
        else:
            mode=None
        self.run = wandb.init(project="looking_good", entity="irosa-ias", group=group_name, id=run_name, config=exp_config,
    ...
    def log_metrics(self, metrics, name_spc):
        wandb.log(metrics)
        wandb.log({"name_spec": name_spc})
