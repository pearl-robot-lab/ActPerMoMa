...
def experiment(cfg: DictConfig = None, cfg_file_path: str = "", seed: int = 0, results_dir: str = ""):
    ...
    ## parameters
    parameters = {'window_sizes': [1, 3],
                  'gain_weighting_factors': [0.1, 0.5, 0.8],
                  'seen_theta_threshs': [0, 30*np.pi/180, 45*np.pi/180],
                  'quality_thresholds': [0.8, 0.85, 0.9],
                  'base_goal_radius_grasps': [0.75, 0.8],
                  'momentums': [400, 500, 600],
                  'deadlock_thetas': [0.0, 3.0 *np.pi/180, 5.0 * np.pi / 180]}

    defaults = {'window_size': 1,
                'gain_weighting_factor': 0.5,
                'seen_theta_thresh': 45*np.pi/180,
                'quality_threshold': 0.9,
                'base_goal_radius_grasp': 0.75,
                'momentum': 500,
                'deadlock_theta': 3.0 * np.pi / 180}
    ...
    for param_combi in param_combinations:
        ...
        logger.strong_line()
        name_spec = ', '.join([key + '=' + str(param_combi[key]) for key in param_combi.keys()])
        ...
        # eval_dataset is a list of tuples respresenting (s, a, r, s', absorbing, info, last); where s is a dict with keys 'tsdf', 'goal_pos', 'view_pos', 'view_theta', 'view_phi', 'obj_states', 'reset_agent'
        name_spec = '_'.join([key + '=' + str(param_combi[key]) for key in param_combi.keys()])
        d_mean, nv_mean, sr, fr, ar = parse_data_and_save(eval_dataset, results_dir, name_spc=f'Algo{algo_name}_' + name_spec)
